...
@Autonomous
class AutoRosuBARIERE : BBLinearOpMode() {
    ...
    override fun runOpMode() {
        ...
        val trajSeq: TrajectorySequence = drive.trajectorySequenceBuilder(startPose)
            ...
            .waitSeconds(1.0)
            .lineToLinearHeading(Pose2d(-5.0, index_rata_y, Math.toRadians(0.0)))
            .addTemporalMarker {
                liftState = LIFT_AUTO.UP
            }
            .waitSeconds(4.0)
            .lineTo(Vector2d(10.0,-68.0))
        ...
        when (liftState) {
            ...
            LIFT_AUTO.UP -> {
                if (timer.milliseconds() > 1500.0) {
                    get<ServoLiftModule>().move_open_auto()
